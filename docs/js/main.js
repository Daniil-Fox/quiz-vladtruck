...
/* harmony import */ var _node_modules_inputmask_dist_inputmask_es6_js__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ../../../node_modules/inputmask/dist/inputmask.es6.js */ "./node_modules/inputmask/dist/inputmask.es6.js");
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (heroSlider) {
    const swiper = new swiper__WEBPACK_IMPORTED_MODULE_0__.Swiper(heroSlider, {
      loop: true,
      speed: 800,
      autoplay: {
        delay: 3000
  ...
  if (contentEl && imgEl) {
    ...
    for (const tel of telInputsAll) {
      try {
        const im = new _node_modules_inputmask_dist_inputmask_es6_js__WEBPACK_IMPORTED_MODULE_3__["default"]("+7 (999) 999-99-99");
        im.mask(tel);
      } catch (e) {
    ...
    const isSlideValid = index => {
      ...
      const requiredRadios = Array.from(requiredEls).filter(el => el.type === "radio");
      const radioNames = {};
      for (const r of requiredRadios) if (r.name) radioNames[r.name] = true;
      for (const name in radioNames) {
        const group = slide.querySelectorAll(`input[type="radio"][name="${name}"]`);
        let ok = false;
        for (const g of group) if (g.checked) {
          ok = true;
          break;
        }
        if (!ok) return false;
    ...
    const isLastSlide = () => contentSlider.activeIndex === contentSlider.slides.length - 1;
    const updateNavState = () => {
      ...
      if (prevBtn) {
        if (contentSlider.activeIndex === 0) prevBtn.setAttribute("disabled", "");else prevBtn.removeAttribute("disabled");
      }
    };
  ...
  if (yearSliderEl) {
    nouislider__WEBPACK_IMPORTED_MODULE_2__["default"].create(yearSliderEl, {
      start: [1990, 2026],
      connect: true,
      range: {
        min: 1990,
